...
class MapArchiveIntegrityMigration
  ...
  sha384_sums_filename = @@archive+"/sha384sums.txt"
  ...
  def image_metadata
    lambda {|d|
      ...
      if d.files? and d.files.any?
        ...
        d.files.map! {|f|
          ...
          if File.exists? imagepath
            ...
            if File.exists? imagemagick_identify_filename
              ...
              if filename_in_identify_file != imagepath
                raise "Bad identify cache: #{imagemagick_identify_filename}"
              end
            ...
            if f[:width].nil? or f[:width].to_i == 0
              f[:width] = m[:width]
            end
            if f[:height].nil? or f[:height].to_i == 0
              f[:height] = m[:height]
            end
            ...
            if f.integrity.to_s == "" or f.integrity =~ /^md(s)?5[:-]/
              ...
              # recheck integrity if file length has changed
              if f[:length] != File.size(imagepath)
                if f.integrity != self.class.sri_integrity(imagepath, "sha384")
                  raise "SHA-384 hash changed for #{imagepath}"
                end
              end
            end
        ...
        log.info "No files: #{d.id} #unrestricted{d.title}"
      end
      d
    }
  end
  ...
  def fix_lang_and_country
    lambda {|d|
      ...
      if comments.any?
        ...
        log.info d.comments.to_jsonunrestricted
      end
